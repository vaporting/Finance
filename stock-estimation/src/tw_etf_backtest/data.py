...
# Project root is three levels up from this file (src/tw_etf_backtest/data.py).
...
def _fix_leverage_discontinuities(lev: pd.Series, base: pd.Series, threshold: float = 0.3) -> pd.Series:
    """Back-adjust undocumented price discontinuities in the leveraged ETF series.

    Daily-leveraged ETFs like 00631L occasionally undergo unit consolidations
    that Yahoo Finance fails to record as a stock split, leaving a single-day
    price cliff with no corresponding split entry. Such a cliff is detected by
    checking whether a day's return tracks roughly 2x the base ETF's same-day
    return; a large unexplained deviation is treated as a data discontinuity,
    and all prior history is rescaled so the series splices back together.
    """
    ...
    for date in aligned.index[deviation.abs() > threshold]:
        ...
        expected_price = prior_price * (1 + 2 * base_ret.loc[date])
...
def load_prices(config: BacktestConfig) -> pd.DataFrame:
    ...
    lev = _download_close(config.lev_ticker, config.start_date, config.end_date)
    base = _download_close(config.base_ticker, config.start_date, config.end_date)
    lev = _fix_leverage_discontinuities(lev, base)
